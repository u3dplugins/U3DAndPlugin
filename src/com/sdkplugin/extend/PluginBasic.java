...
public class PluginBasic extends AbsU3DListener {
	...
	public void receiveFromUnity(String json) throws Exception {
		...
		try {
			...
			if (isThrow) {
				...
				ex.printStackTrace();
				Tools.msg2U3D(CODE_ERROR, ex.getMessage(), obj);
			}
		} finally {
			reInit();
		}
	...
	private void handlerJson(final String cmd, JSONObject data) throws Exception {
		switch (cmd) {
		case "getPackageInfo":
			Tools.msg2U3D(Tools.getTextInAssets(getCurContext(), data.getString("filename")));
			break;
		case "kill":
			killSelf();
			break;
		case "logLev":
			...
			break;
		case "phoneInfo":
			Tools.msg2U3D(CODE_SUCCESS, "", cmd, getPhoneState(mapData));
			break;
		case "phone_mem_cpu":
			Tools.msg2U3D(CODE_SUCCESS, "", cmd, getMemCpuInfo(mapData));
			break;
		default:
			handlerMsg(cmd, data);
			break;
	...
	protected void handlerMsg(final String cmd, JSONObject data) throws Exception {
		...
		case "isInApk":
			...
			Tools.msg2U3D(CODE_SUCCESS, "", cmd, data);
			break;
		case "shareImg":
			// 图片分享
			...
			break;
		default:
			Tools.msg2U3D(CODE_SUCCESS, "", String.format("{\"cmd\":\"%s\",\"val\":\"这是一个demo\"}", cmd));
			break;
		}
	}
